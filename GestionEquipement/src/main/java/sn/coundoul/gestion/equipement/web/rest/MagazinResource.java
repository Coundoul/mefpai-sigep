...
/**
 * REST controller for managing {@link sn.coundoul.gestion.equipement.domain.Magazin}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class MagazinResource {
    ...
    private static final String ENTITY_NAME = "gestionEquipementMagazin";
    ...
    public ResponseEntity<Magazin> updateMagazin(
     ...
     */
    @GetMapping("/magazins")
    public ResponseEntity<List<Magazin>> getAllMagazins(Pageable pageable) {
        log.debug("REST request to get a page of Magazins");
        Page<Magazin> page = magazinRepository.findAll(pageable);
