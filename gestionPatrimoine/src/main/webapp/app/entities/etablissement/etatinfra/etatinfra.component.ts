...
@Component({
  selector: 'jhi-etatinfra',
  templateUrl: './etatinfra.component.html',
  styleUrls: ['./etatinfra.component.scss']
})

export class EtatinfraComponent implements OnInit {
  batiments?: IBatiment[];
  etablissement!: any;
  ...
  protected onSuccess(data: IBatiment[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    ...
    this.page = page;
    if (navigate) {
      this.router.navigate(['/batiments/etatinfra'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
