...
@Component({
  selector: 'jhi-detenteur',
  templateUrl: './detenteur.component.html',
})
export class DetenteurComponent implements OnInit {
  ...
  constructor(
    protected detenteurService: DetenteurService,
  ...
  delete(detenteur: IDetenteur): void {
    const modalRef = this.modalService.open(DetenteurDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IDetenteur[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    ...
    this.page = page;
    if (navigate) {
      this.router.navigate(['/detenteur'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
