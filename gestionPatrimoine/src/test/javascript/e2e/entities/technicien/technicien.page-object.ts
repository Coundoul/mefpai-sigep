...
export class TechnicienUpdatePage {
  ...
  chefMaintenanceSelect = element(by.id('field_chefMaintenance'));
  ...
  async chefMaintenanceSelectLastOption(): Promise<void> {
    await this.chefMaintenanceSelect.all(by.tagName('option')).last().click();
  }

  async chefMaintenanceSelectOption(option: string): Promise<void> {
    await this.chefMaintenanceSelect.sendKeys(option);
  }

  getChefMaintenanceSelect(): ElementFinder {
    return this.chefMaintenanceSelect;
  }

  async getChefMaintenanceSelectedOption(): Promise<string> {
    return await this.chefMaintenanceSelect.element(by.css('option:checked')).getText();
  }
