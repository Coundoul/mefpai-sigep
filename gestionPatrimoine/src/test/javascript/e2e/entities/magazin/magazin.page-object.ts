...
export class MagazinComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-magazin div table .btn-danger'));
  title = element.all(by.css('jhi-magazin div h2#page-heading span')).first();
...
export class MagazinUpdatePage {
  pageTitle = element(by.id('jhi-magazin-heading'));
  saveButton = element(by.id('save-entity'));
  cancelButton = element(by.id('cancel-save'));

  idInput = element(by.id('field_id'));
  nomMagazinInput = element(by.id('field_nomMagazin'));
  ...
  async setNomMagazinInput(nomMagazin: string): Promise<void> {
    await this.nomMagazinInput.sendKeys(nomMagazin);
  }

  async getNomMagazinInput(): Promise<string> {
    return await this.nomMagazinInput.getAttribute('value');
  }
...
export class MagazinDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-magazin-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-magazin'));
