...
export class MangasinierFichisteComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-mangasinier-fichiste div table .btn-danger'));
  title = element.all(by.css('jhi-mangasinier-fichiste div h2#page-heading span')).first();
...
export class MangasinierFichisteUpdatePage {
  pageTitle = element(by.id('jhi-mangasinier-fichiste-heading'));
  ...
  comptablePrincipaleSelect = element(by.id('field_comptablePrincipale'));
  ...
  async comptablePrincipaleSelectLastOption(): Promise<void> {
    await this.comptablePrincipaleSelect.all(by.tagName('option')).last().click();
  }

  async comptablePrincipaleSelectOption(option: string): Promise<void> {
    await this.comptablePrincipaleSelect.sendKeys(option);
  }

  getComptablePrincipaleSelect(): ElementFinder {
    return this.comptablePrincipaleSelect;
  }

  async getComptablePrincipaleSelectedOption(): Promise<string> {
    return await this.comptablePrincipaleSelect.element(by.css('option:checked')).getText();
  }
...
export class MangasinierFichisteDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-mangasinierFichiste-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-mangasinierFichiste'));
