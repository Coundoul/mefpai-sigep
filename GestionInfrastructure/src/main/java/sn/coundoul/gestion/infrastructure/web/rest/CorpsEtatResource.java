...
/**
 * REST controller for managing {@link sn.coundoul.gestion.infrastructure.domain.CorpsEtat}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CorpsEtatResource {
    ...
    private static final String ENTITY_NAME = "gestionInfrastructureCorpsEtat";
    ...
    public ResponseEntity<CorpsEtat> createCorpsEtat(@Valid @RequestBody CorpsEtat corpsEtat) throws URISyntaxException {
        ...
        CorpsEtat result = corpsEtatRepository.save(corpsEtat);
    ...
    public ResponseEntity<CorpsEtat> updateCorpsEtat(
        ...
        if (!corpsEtatRepository.existsById(id)) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, corpsEtat.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<CorpsEtat> partialUpdateCorpsEtat(
        @PathVariable(value = "id", required = false) final Long id,
        ...
        Optional<CorpsEtat> result = corpsEtatRepository
            ...
            .map(corpsEtatRepository::save);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, corpsEtat.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<CorpsEtat>> getAllCorpsEtats(Pageable pageable) {
        log.debug("REST request to get a page of CorpsEtats");
        Page<CorpsEtat> page = corpsEtatRepository.findAll(pageable);
    ...
    public ResponseEntity<CorpsEtat> getCorpsEtat(@PathVariable Long id) {
        log.debug("REST request to get CorpsEtat : {}", id);
        Optional<CorpsEtat> corpsEtat = corpsEtatRepository.findById(id);
    ...
    public ResponseEntity<Void> deleteCorpsEtat(@PathVariable Long id) {
        ...
        corpsEtatRepository.deleteById(id);
        return ResponseEntity
            .noContent()
